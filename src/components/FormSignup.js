...
const FormSignup = ({ sessionObject }) => {
  ...
  // ***********
  console.log("OLD",myForm.pageState)
  ...
  useEffect(() => {
    myForm.setPageStateByField("destination_name", myForm.pageState.station_to_name);
  }, [myForm.pageState.station_to_name])

  useEffect(() => {
    myForm.setPageStateByField("destination", myForm.pageState.station_to);
  ...
  useEffect(() => {
    setRoundAmount();
    console.log("TOTAL AMT",myForm.pageState.new_total_amount)
  },[myForm.pageState.new_total_amount])
  
  const setRoundAmount = () => {

    let roundAmount = parseInt(myForm.pageState.new_total_amount);

    if(parseInt(myForm.pageState.bilty_charge)> 0 && 
      parseInt(myForm.pageState.hamali) > 0 && 
      parseInt(myForm.pageState.other_amount) > 0 &&
      parseInt(myForm.pageState.door_del_charge) > 0 &&
      myForm.pageState.bilty_type!="To Be Billed"
    ){
      ...
      myForm.pageState.new_total_amount = roundAmount.toString();
      myForm.pageState.total_amount = myForm.pageState.new_total_amount;
    }  
    // return roundAmount;
  }
  ...
  //  CODE FOR SETTING THE VALUE OF AMOUNT FIELD WITH RESPECT TO UNIT VALUES

   useEffect(()=> {
    

   },[myForm.pageState["item_in"]]);
 
  //  AMOUNT CHANGE ENDS

  const freightCalculation = () => {
    for(let i = 0 ; i < myForm.pageState["item_in"].length ; i++) {
      if(myForm.pageState["item_in"][i].unit == 'W'){
        myForm.pageState["item_in"][i].amt = myForm.pageState["item_in"][i].c_wgt * myForm.pageState["item_in"][i].rate;
        myForm.pageState.new_freight = myForm.pageState["item_in"][i].amt.toString();
        myForm.pageState.new_total_amount = myForm.pageState.new_freight;        
      }
  
      else if(myForm.pageState["item_in"][i].unit == 'C'){
        myForm.pageState["item_in"][i].amt = myForm.pageState["item_in"][i].pkgs * myForm.pageState["item_in"][i].rate;
        myForm.pageState.new_freight = myForm.pageState["item_in"][i].amt.toString();
        myForm.pageState.new_total_amount = myForm.pageState.new_freight;
      }
  
      else if(myForm.pageState["item_in"][i].unit == 'T' || myForm.pageState["item_in"][i].unit == 'F'){
        myForm.pageState["item_in"][i].amt = 1 * myForm.pageState["item_in"][i].rate;
        myForm.pageState.new_freight = myForm.pageState["item_in"][i].amt.toString();
        myForm.pageState.new_total_amount = myForm.pageState.new_freight;
      }
    }
  ...
  // CODE FOR SETTING THE TOTAL AMOUNT
  let p = myForm.pageState
  myForm.pageState.new_total_amount = (parseInt(p.new_freight)||0)+(parseInt(p.bilty_charge)||0)+(parseInt(p.hamali)||0)+(parseInt(p.door_del_charge)||0)+(parseInt(p.other_amount)||0)
  const roundAmountTotalFunction = () => {
    let temp = p.new_total_amount;
    while(temp%5!=0){
      temp++;
    }
    myForm.pageState.round_off = temp - p.new_total_amount;
    myForm.pageState.new_total_amount = temp;

    console.log("NEW TOTAL AMOUNT",myForm.pageState.new_total_amount);
  }

  roundAmountTotalFunction();

  //  useEffect(()=> {
  //   console.log("BILTY", parseInt(myForm.pageState.bilty_charge))
  //   let length = myForm.pageState.bilty_charge.length;
  //   if(length == 1) {
  //     myForm.pageState.new_total_amount += parseInt(myForm.pageState.bilty_charge);
  //   }
  //   else {
  //     let extra = 0;
  //     for(let i = 0 ; i < length-1 ; i++) {
  //       extra += myForm.pageState.bilty_charge[i];
  //     }

  //     myForm.pageState.new_total_amount += parseInt(myForm.pageState.bilty_charge) - extra;
  //   }
  //   console.log("LENGTH",length)
    
  //  },[myForm.pageState.bilty_charge]);



  //  useEffect(()=> {
  //   console.log("HAMALI", parseInt(myForm.pageState.hamali))
  //   let length = myForm.pageState.hamali.length;
  //   if(length == 1) {
  //     myForm.pageState.new_total_amount += parseInt(myForm.pageState.hamali);
  //   }
  //   else {
  //     let extra = 0;
  //     for(let i = 0 ; i < length-1 ; i++) {
  //       extra += myForm.pageState.hamali[i];
  //     }

  //     myForm.pageState.new_total_amount += parseInt(myForm.pageState.hamali) - extra;
  //   }
  //   console.log("LENGTH",length)
  //  },[myForm.pageState.hamali]);



  //  useEffect(()=> {
  //   let length = myForm.pageState.door_del_charge.length;
  //   if(length == 1) {
  //     myForm.pageState.new_total_amount += parseInt(myForm.pageState.door_del_charge);
  //   }
  //   else {
  //     let extra = 0;
  //     for(let i = 0 ; i < length-1 ; i++) {
  //       extra += myForm.pageState.door_del_charge[i];
  //     }

  //     myForm.pageState.new_total_amount += parseInt(myForm.pageState.door_del_charge) - extra;
  //   }
  //   console.log("LENGTH",length)
  //  },[myForm.pageState.door_del_charge]);



  //  useEffect(()=> {
  //   let length = myForm.pageState.other_amount.length;
    
  //   if(length == 1) {
  //     myForm.pageState.new_total_amount += parseInt(myForm.pageState.other_amount);
  //   }
  //   else {
  //     let extra = 0;
  //     for(let i = 0 ; i < length-1 ; i++) {
  //       extra += myForm.pageState.other_amount[i];
  //     }

  //     console.log("LEN",length);
  //   console.log("TOTAL1",myForm.pageState.new_total_amount);
  //   console.log("AMOUNT",myForm.pageState.other_amount);

  //     myForm.pageState.new_total_amount += parseInt(myForm.pageState.other_amount) - extra;
  //     console.log("TOTAL2",myForm.pageState.new_total_amount);
  //   }
  //  },[myForm.pageState.other_amount]);

// CODE ENDS FOR TOTAL AMOUNT
  ...
  const checkVisibilityCondition = (fieldInfo) => {
    // console.log("FIELD INFO", myForm.pageState)
  ...
  const fetchChargeRate = async () => {
    ...
    myForm.makeFocusOnParticularField("charge_wgt");
    ...
    <div className="form-container">
      ...
      <div className="form">
        ...
        <div className="form-footer">
          ...
          <button
            onClick={(e) => {
              myForm.setClearDataOnSave(false);
