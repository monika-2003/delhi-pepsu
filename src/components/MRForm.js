...
const MRForm = ({ sessionObject }) => {
  ...
  const roundOffAmount = () => {
    ...
    console.log("PULKIT_TOTAL_AMOUNT",myForm.pageState.total_amount)
    let temp = myForm.pageState.total_amount;
    while(temp%5!=0){
      temp++;      
    }
    console.log("PULKIT_TEMP",temp);
    myForm.pageState.round_off = temp - myForm.pageState.total_amount;
    myForm.pageState.total_amount = temp;
  }

  useEffect(() => {
    roundOffAmount();
  },[myForm.pageState.total_amount])
  ...
  const linkBilty = async (e) => {
    ...
    if (e.key == "Enter") {
      ...
      console.log("PULKIT_SUFFIX",myForm.pageState)
      ...
      // to set data in pagestate from temp_response
      if(temp_response.pay_type_name == "To Pay" ) {
        myForm.pageState.to_pay_amount = temp_response.total_amount;
      } else {
        myForm.pageState.to_pay_amount = 0;
      }
  ...
  const checkVisibilityCondition = (fieldInfo) => {
    if (fieldInfo.name == "edit_button" && myForm.pageState.role_id != "1") {
      ...
      fieldInfo.name == "cheque_no" &&
      (myForm.pageState.payment_type == "3")
    ) {
      return false;
    } else {
      return true;
