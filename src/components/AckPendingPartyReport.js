...
function AckPendingPartyReport({ sessionObject }) {
  ...
  const fetchData = React.useCallback(
    async ({ pageSize, pageIndex, sortBy, customFilters, dateObject }) => {
      ...
      if (fetchId === fetchIdRef.current) {
        ...
        // );
          if (dateObject.all_branch == "2") {
            newObject.created_from = parseInt(
              sessionObject.sessionVariables.branch_id
            );
          }
        // }
        ...
        let response = await fetch(SERVER_URL + "/report/ack_pending", {
          method: "POST",
          headers: {
...
export default AckPendingPartyReport;
