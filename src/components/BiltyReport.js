...
function BiltyReport({ sessionObject }) {
  ...
  console.log("USE SESSION",sessionObject);
...
//   })
  ...
  const fetchData = React.useCallback(
    async ({ pageSize, pageIndex, sortBy, customFilters, dateObject }) => {
      ...
      console.log("PULKIT_CUSTOM_FILTERS", sortBy, customFilters, fetchId);
      ...
      if (fetchId === fetchIdRef.current) {
        ...
        console.log("PULKIT_FILTERS",newObject," AND CUSTOM FILTER",customFilters)
        newObject = {...newObject,...customFilters}

        console.log("PULKIT[_]FILTERS_AFTER_RESULT",newObject)
        ...
        let url = SERVER_URL;
        if(!newObject.multi_station) {
          url += "/report/"
        }
        else {
          url += "/report/multi_station"
        }
        ...
        if(dateObject.report_type != "inward") {
          if(resp.branch_info.branch_from_list == undefined || resp.branch_info.branch_from_list.length == 0) {
            localStorage.setItem('FromList', []);
          }
          else {
            localStorage.setItem('FromList', JSON.stringify(resp.branch_info.branch_from_list));
          }

          if(resp.branch_info.branch_to_list == undefined || resp.branch_info.branch_to_list.length == 0) {
            localStorage.setItem('ToList', []);
          }
          else {
            localStorage.setItem('ToList', JSON.stringify(resp.branch_info.branch_to_list));
          }
      }

        // let fromStation = JSON.stringify(resp.branch_info.branch_from_list);
        // let toStation = JSON.stringify(resp.branch_info.branch_to_list);
