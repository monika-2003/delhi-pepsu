...
import {clearBilty, popBilty} from "../slices/challanBiltySlices"
import { clearUnloadingBilty, pushUnloadingBilty } from "../slices/unloadingSlices";
...
const useForm = (formName, validate, dataObject, apiConfig) => {
    ...
    const tempData = useSelector(state => state.unloading_bilty_reducers.unloading_bilty_arr);

    const [pageState, setPageState] = useState(dataObject);

    const [initialpageState, setInitialPageState] = useState(dataObject); //NEW CODFE FOR SENDING CLERANCE STATUS
    ...
    // ****************
    const weight1 = useRef(0)

    // For Calculating ActualWeight and ChangedWeight;    
    ...
    // useEffect(() => {
    //     setActArr([...actArr,parseInt(pageState.item_in[0].weight)]);
    //     let c = parseInt(pageState.item_in[0].weight);
    //     if(c > 0){
    //         while(c%5!=0) c++;
    //         setCArr([...cArr,c]);       
    //     }
    // },[pageState.item_in[0].weight])
    ...
    // useEffect(() => {
    //     console.log("PULKIT_TEST_TEMP_DATA",tempData)
    //     pageState.unloading_bilty_table.push(...tempData)
    //     console.log("PULKIT_PAGESTATE FROM USEFORM",pageState)
    // },[tempData])
    ...
    const handleChange = async (e) => {
        ...
        console.log("Temp ", name, value,"pageState -> ",pageState);
        setPageState({
            ...pageState,
            [name]: value,
        });
    ...
    async function handleSuccess() {
        ...
        console.log("TEST...",submitObject)

        console.log("PULKIT_TEST",submitObject," And Form name->",formName);
        
        submitObject.destination = submitObject.station_to;
        ...
        console.log("PULKIT_FORM_NNAME",formName);
        /**
         * SUbmit object adding extra values.
         */
        if (formName == "Bilty") {
            // when hits then it will add total amount and decrypt new_total_amount
            submitObject.total_amount = submitObject.new_total_amount
            console.log("PULKIT_AFTER_FORM[+]",submitObject)
            // localStorage.setItem("total_amount",submitObject.total_amount)
            // localStorage.setItem("new_total_amount",submitObject.new_total_amount)
            console.log("TEST...BILTY",submitObject);
            if (pageMode == "write") {
                let apiObj = {
    ...
    async function suggestionFetchApi(fieldInfoObject, value, additionalInfo) {
         ...
         * field Info Object
         */
        console.log("VALUES",{fieldInfoObject, value, additionalInfo});
        const fieldName = fieldInfoObject["name"];
        ...
        console.log("PULKIT_USEFORM_RESPONSE",responseObject,"ANd inputData",inputData);
        return responseObject;
    }

    const changingValues = (newValue , i , w1 , w2) => {
        let c1 = newValue[i].weight;
        ...
        if(newValue[i].c_wgt >= 50) {
            pageState.charge_wgt = w2;
        }
        pageState.actual_weight = w1;                    
        
    }

    const setPageStateByField = (name, newValue) => {
        console.log("TET_JUST_CLEAR",name," -> ",newValue);
        if(name === "delete_redux_id"){          
            dispatch(popBilty(newValue))          
        }

        let w1 = 0 , w2 = 0;
        if(newValue!= undefined) {
            for(let i = 0 ; i < newValue.length ; i++) {
                if(newValue[i].weight) {
                    w1 += parseInt(newValue[i].weight);
                    let c = newValue[i].weight;
                    while(c % 5 != 0) {
                        c++;
                    }
                    if(c <=50) {
                    ...
                    changingValues(newValue , i , w1, w2 );
                }
    ...
    const handleSubmit = (e) => {
        if(pageState.payment_type == '1') {
            pageState.cheque_no = null;
            pageState.account_name = null;
        }
        else if(pageState.payment_type == '2') {
            pageState.account_name = null;
        }
        else if(pageState.payment_type == '3') {
            pageState.cheque_no = null;
        }
        ...
        console.log("PULKIT_STATE",pageState,"and failed test case",internalErrorsDummy);
    ...
    const getSuffixesOfBilty = async (bilty_no, fyear) => {
        ...
        if(resp.ok) {
            ...
            console.log("MONIKA_SUFFIX_OBJECT",suffixObj);
    
            // step - 5
            // calling getSuffix directly if the length of suffix object is only one
            if("options" in suffixObj && suffixObj.options.length == 1) {
                console.log("HERE IN LENGTH1");
                getSuffix(suffixObj.options[0]);
            }
            // step - 6
            // calling getSuffix directly if the length of suffix object is more than one
            if("options" in suffixObj && suffixObj.options.length > 0) {
              suffixObj.options = suffixObj.options.map(suff => {
                  return (suff == null ? "" : suff)
              });
              console.log("HERE IN LENGTH GREATER");
              // console.log(suffixObj.options);
                ...
                // myForm.pageState.suffix_options = suffixObj.options;
                // myForm.pageState.suffix = suffixObj.options[0];
                
                // step - 7
                // calling func to display suffix popup
                displaySuffix();
        ...
        // step - 10
    // after clicking on the suffix provided, last api is called to get the value related to specific fyear and suffix
    const getSuffix = async (suffix) => {
        console.log("PULKIT_SUFFIX",suffix);
        let suff = suffix.suffix
        if(suff === undefined){
            suff = suffix;
        }
        console.log("PULKIT_TEST",formName)
        if(formName === "MR"){
            pageState.bilty_no = pageState['Bilty No']
        }
        const url =
            SERVER_URL +
            "/bilty/" +
            ...
            "&suffix=" +
            suff +
            "&fyear=" + pageState.fYear + "&companyId=" + pageState.company_id;
        const response = await fetch(url);
        const respData = await response.json();
        console.log("PULKIT_RESPONSE",respData);

        if(respData.station_to){
            console.log("PULKIT-SETS_HERE",respData.station_to);
          
            setPageStateByField("station_to",respData.station_to);
            setPageStateByField("suffix",respData.suffix);          
        }
    }
    ...
    const getFyearsOnKeyEnter = async (e, myFormName, page_id) => {       
        if(e.key == "Enter") {
            ...
            let dataToSend = {};
            let finalFyearList = [];
            console.log("TEMP_FORM_NAME",myFormName);
            ...
            if(finalFyearList.length == 1) {
                if(myFormName == "Bilty Inquiry") {
                    ...
                    console.log("HERE2"); 
                    console.log("this new",pageState["enterEvent"], e.target);
    ...
    const displayFyearPopup = (close) => (
        ...
        // step - 8
        // display suffix
        const displaySuffix = (close) => (
            <div className="pop-up-container">
                <div className="pop-up-header">
                    <div>Please Choose Suffix</div>
                    <div>
                        <a className="pop-up-close btn" onClick={() => {
                            setPageStateByField("fyearList", []);
                            close();
                    ...
                    {pageState.suffix_options.map(suffix => {
                        return <button
                            style={{
                                fontSize: "20px",
                            }}  
                            ...
                            value={suffix}
                            onClick={(e) => {
                                setPageStateByField("suffix_options", []);
                                getSuffix({suffix});
                                close();
                            }}
                        >{suffix}</button>
    ...
    const getPageOnKeyEnter = (e, page_id, popupFyear) => {
        ...
        console.log("PULKIT_GET_PAGE",e," AND PAGE_ID",page_id," AND POP_UP",popupFyear)
        ...
        if (e.key == "Enter") {
            console.log("PULKIT_FORMNAME",formName);
            if (formName == "Bilty") {
    ...
    const fetchLastData = async () => {
        ...
        if (formName == "Bilty") {
            ...
            if (!response.ok) {
                ...
                console.log("useForm PAGESTATE", pageState)
                resp.item_in = [
                    {
                        amount: "",
                        item_name: "",
        ...
        if (formName == "Challan") {
            ...
            let fYear = JSON.parse(sessionStorage.getItem("financial_year_for_fetch")).financial_year_for_fetch;
            const url2 = SERVER_URL + "/unloading/?branch_id="+sessionObject.sessionVariables.branch_id+"&fyear="+fYear;
            const response2 = await fetch(url2);
    
    
            // const url2 = SERVER_URL + "/unloading/?branch_id="+sessionObject.sessionVariables.branch_id+"&fyear="+fYear_fetch;
            // const response2 = await fetch(url2);

            // let unloading = [];

            // let resp = await response2.json();
            // console.log("PULKIT_TOTAL_RESPONSE",resp);
            // for(let i=0;i<resp.data.length;i++){
            //     unloading.push(resp.data[i]);
            // }

            // setPageState({
            //     ...pageState,
            //     ["unloading_bilty_table"]:unloading
            // });
            ...
            if (!response.ok) {
                ...
                console.log("PULKIT_TABLE",resp2.data)
                const arr = []
                
                for(let i=0;i<resp2.data.length;i++){
                    if(resp2.data[i].excess>0){
                        arr.push(resp2.data[i])
                    }
                }
                ...
                setPageState({
                    ...
                    ["unloading_bilty_table"]:arr
                });
            }


            // if (!response2.ok) {
            //     setPageStateByField("unloading_bilty_table", "Error");
            //     return;
            // } else {
            //     let unloading = [];
            //    
        ...
        if (formName == "ChallanInward") {
            ...
            console.log("PULKIT_TEMPDATA",pageState)
            // const tempState = [...resp2.data,...tempData]
            console.log("RESP",resp);
            dispatch(clearUnloadingBilty())
            for(let i=0;i<resp2.data.length;i++)
                dispatch(pushUnloadingBilty(resp2.data[i]))
            setPageStateByField("unloading_bilty_table",resp2.data);
            
            // setPageStateByField("unloading_bilty_table", "");
